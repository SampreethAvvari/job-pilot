...
- **Cloud Run Job `jobpilot`** — the pipeline. Built from repo source via Cloud Build,
  ...
  `run.invoker` + `run.developer` on itself for the scheduler — `run.developer` because
  the fast-fetch trigger passes container-arg overrides, which need
  `run.jobs.runWithOverrides`; plain `run.invoker` 403s, see BL-21). Env
  `GOOGLE_CLOUD_PROJECT` switches the scorer/tailor to Vertex AI Gemini (no API key
  needed). Task timeout 45m.
...
- **Cloud Scheduler** — `jobpilot-daily` (`0 0,6,12,18 * * *`, full runs) and
  `jobpilot-hourly` (`15,45 * * * *`, `--fast` + free sources only — every 30 min,
  offset from the :00 full runs). Both run the inbox watch (reply detection +
  alerts) — no extra scheduler needed.
