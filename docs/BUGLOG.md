...
### BL-23 · Outreach drafts: generic, unsigned, recipient-less (Apollo silently dead)
- **Symptom:** every drafted row said "no contact found"; drafts had no To address, no candidate name, no links, no resume attached, and read like business-speak.
- **Root causes:** (1) Apollo deprecated `mixed_people/search` — it returns 422, which the code treated as "free-plan limit, skip silently", so every contact lookup failed for weeks; the replacement `mixed_people/api_search` hides name/email until a `people/match` reveal. (2) The draft prompt never received the candidate's name — the LLM had nothing to sign with. (3) No signature/links/attachment support existed. (4) Drafts were created even with no recipient.
- **Fix:** `api_search` + capped `people/match` reveals (MAX_REVEALS=3 credits/company); contacts without an email are dropped; **no recipient email → no draft, ever** (Contact column explains + Find people link). Deterministic signature (name + portfolio/LinkedIn/GitHub from new Profile fields, secret v5); tailored-resume PDF attached from Drive; prompt rewritten (≤110 words, technical specifics only, business/revenue metrics banned).
- **Guard:** a 4xx from a third-party API is not always "plan limit" — log or distinguish deprecation errors; when a lookup quietly returns [], make the empty case visible in the Sheet (it was, which is how the user caught it).
